...
float AverageCarPrice(std::vector<data>& values){
    ...
    */
    ...
    for(int index=0;index < Length; index++){
        if(values[index]["Price"]==-1)
        continue;
       counter++;
       Total+=values[index]["Price"];
    }
...
int MaxHorsepower(std::vector<data>& values){
    ...
    for(int index=0;index < Length; index++){
        if(values[index]["Price"]==-1)
            continue;
        if(MaxPower<values[index]["Horsepower"])
            MaxPower=values[index]["Horsepower"];
...
int FindLowestEfficiency(std::vector<data>& values){
    ...
    */
    ...
    for(int index=0;index < Length; index++){
        if(values[index]["Efficiency"]==-1)
            continue;
        if(LowestEfficiency>values[index]["Efficiency"])
            LowestEfficiency=values[index]["Efficiency"];
